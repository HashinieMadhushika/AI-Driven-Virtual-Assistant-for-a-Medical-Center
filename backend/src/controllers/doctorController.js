...
export const deleteDoctorImage = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const urlParts = doctor.profileImageUrl.split('/');
    const fileName = urlParts[urlParts.length - 1].split('.')[0];
    const publicId = `medical-center/doctors/${fileName}`;

    // Delete from Cloudinary
    const cloudinaryService = await import('../services/cloudinaryService.js');
    await cloudinaryService.deleteDoctorImage(publicId);

    // Update doctor to remove image URL
    await doctor.update({ profileImageUrl: null });
