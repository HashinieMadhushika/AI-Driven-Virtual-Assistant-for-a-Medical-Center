...
export default function LoginPage() {
  ...
  // (UI only) role selector — we will NOT trust this for navigation
  const [role, setRole] = useState<Role>('admin')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ Navigate based on BACKEND role (not UI selected role)
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-100 p-8">
        ...
        <div className="mt-6">
          ...
          <p className="mt-2 text-[11px] text-slate-500">
            Note: Role is determined by your account in the database (backend). This selector is only for UI.
          </p>
        </div>
