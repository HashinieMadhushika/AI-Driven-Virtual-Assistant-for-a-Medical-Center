    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100">
        <div className="flex items-center justify-between">
          ...
          <button className="px-6 py-3 bg-teal-700 text-white font-semibold rounded-xl hover:bg-teal-800 transition-all shadow-sm">
            Ask AI Assistant
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
          ...
          <p className="text-3xl font-bold text-slate-800 mt-1">{stats.pendingReviews}</p>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
          ...
          <p className="text-3xl font-bold text-slate-800 mt-1">{stats.notifications}</p>
        </div>
      </div>
            ...
            {loadingAppointments ? (
              ...
              todaysSchedule.map((appointment) => {
                ...
                return (
                  <div
                    key={appointment.id}
                    className="flex items-center justify-between p-4 border border-slate-100 rounded-xl hover:shadow-md transition-shadow"
                  >
                    <div className="flex items-center space-x-4">
                      <div className="w-10 h-10 bg-linear-to-br from-teal-700 to-cyan-500 rounded-full flex items-center justify-center text-white font-bold">
                        {patientInitial}
                      </div>
                      <div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-slate-800 flex items-center">
                <svg className="w-5 h-5 mr-2 text-teal-700" fill="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex items-center space-x-2">
                {isCalendarConnected && (
                  <a
                    ...
                    className="text-xs px-2 py-1 bg-teal-700 text-white rounded hover:bg-teal-800 transition-colors"
                  >
                    Add Event
                  </a>
                )}
            ...
            {!isCalendarConnected ? (
              <div className="text-center py-8">
                ...
                <p className="text-slate-500 text-sm mb-4">Connect Google Calendar to see your events</p>
                <a href="/doctor/profile" className="text-teal-700 hover:text-teal-800 text-sm font-medium">
              ...
              <>
                ...
                <div className="grid grid-cols-7 gap-1 text-center text-sm">
                  {['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'].map((day) => (
                    <div key={day} className="text-slate-500 font-medium text-xs py-2">{day}</div>
                  ))}
                    ...
                    return (
                      <div
                        ...
                        className={`min-h-[50px] p-1 rounded-lg relative ${
                          !day ? '' :
                          today ? 'bg-teal-700 text-white font-bold' : 
                ...
                {calendarEvents.filter(event => {
                  ...
                  <div className="mt-4 space-y-2">
                    <p className="text-xs font-semibold text-slate-600">Today's Events:</p>
                    {calendarEvents.filter(event => {
                      const eventDate = new Date(event.start.dateTime || event.start.date);
                      const today = new Date();
                      return eventDate.toDateString() === today.toDateString();
                    }).slice(0, 2).map((event, idx) => (
                      <div key={idx} className="p-2 bg-teal-50 rounded-lg">
                        <p className="text-xs font-bold text-teal-800">{event.summary}</p>
            ...
            <div className="space-y-4">
              {announcements.map((announcement) => (
                <div key={announcement.id} className="flex items-start space-x-3">
                  <div className="w-10 h-10 bg-teal-50 rounded-full flex items-center justify-center text-teal-700 font-bold text-sm shrink-0">
                    {announcement.author.charAt(0)}
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-slate-800">{announcement.author}</p>
